...
export class ProductosService {
  ...
  async create(
    ...
    try {
      ...
      return {
        ...
        precio_antes_oferta: nuevoProducto.precio_antes_oferta,
  ...
  async findBultoCerradoProducts(
    paginationDto: PaginationDto,
  ): Promise<GetProductosResponse> {
    const { limit = 10, offset = 0 } = paginationDto;
    try {
      ...
      this.logger.error(error);
      throw new InternalServerErrorException(
        'Error al buscar los productos por bulto cerrado',
        error,
      );
    }
  }
  ...
  private plainProduct(producto: Producto): ProductoPlainResponse {
    return {
      ...
      precio_antes_oferta: producto.precio_antes_oferta,
